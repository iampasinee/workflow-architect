  ...
  return <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); saveStudent(); }}>
    ...
    <section className="space-y-3 border-t pt-4">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <label className="text-xs font-semibold">ปีเข้า<select value={form.admissionYear} onChange={(event) => {
        ...
        }} className={`${inputClass} mt-1`}><option value="">เลือกปีเข้า</option>{getAdmissionYearOptions().map((year) => <option key={year} value={year}>{getAdmissionCode(year)}</option>)}</select><span className="mt-1 block font-normal text-slate-500">จัดเก็บเป็นปีการศึกษาเต็ม เช่น 67 = 2567</span></label>
...
export const StudentManagement: React.FC = () => {
  ...
  const exportCsv = () => {
    const header = ['รหัสนักศึกษา', 'ชื่อ-นามสกุล', 'อีเมล', 'คณะ', 'ภาควิชา', 'สาขาวิชา', 'ปีเข้า', 'ชั้นปี', 'กลุ่มเรียน', 'สถานะ'];
  ...
  return <div className="min-w-0 space-y-5">
    ...
    <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="inline-flex w-full rounded-xl bg-slate-100 p-1 sm:w-auto" role="group" aria-label="รูปแบบการกรองนักศึกษา">
          {[
            ...
            { value: 'admission' as const, label: 'แยกตามปีเข้า', icon: CalendarRange },
      ...
      {filterMode === 'all' ? <div className="grid items-end gap-3 lg:grid-cols-[minmax(0,1fr)_220px_220px]">
        ...
        <div><h2 className="text-sm font-bold text-slate-900">ตัวกรองข้อมูลการศึกษา</h2><p className="mt-1 text-xs text-slate-500">เลือกคณะ ภาควิชา สาขาวิชา และปีเข้าเพื่อกรองนักศึกษา</p></div>
        ...
        <div className="grid items-end gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto]">
          <label className="space-y-1 text-xs font-semibold text-slate-700"><span>ปีเข้า</span><select aria-label="ปีเข้า" value={admissionFilter} onChange={(event) => { setAdmissionFilter(event.target.value); setGroupFilter(''); }} className={inputClass}><option value="">ทุกปีเข้า</option>{admissionYears.map((year) => <option key={year} value={year}>ปีเข้า {getAdmissionCode(year)}</option>)}</select></label>
