...
import {
  courseDeleteError, courseOfferingSettings, courseStudentCount, findSection, sectionDeleteError,
  sectionIdOf,
} from '../../services/courseState';
import { academicSettings } from '../../utils/academicYear';
import { academicPath, isAcademicPathActive } from '../../services/academicState';
...
const emptySectionForm = (courseId = ''): SectionInput => ({
  ...
  academicYear: academicSettings.currentAcademicYear, primaryTeacherId: '', coTeacherIds: [], groupIds: [], status: 'active',
});
...
export const CoursesAndSectionsPage: React.FC = () => {
  ...
  const enrolledGroups = new Set(activeSections.flatMap(({ section }) => section.groupIds || []));
  const metrics = [
    ...
    { label: 'กลุ่มเรียนที่ลงทะเบียน', value: enrolledGroups.size, unit: 'กลุ่มเรียน', icon: Users, color: 'text-violet-600', iconBg: 'bg-violet-50' },
  ...
  const openSection = (located?: { course: Course; section?: Section }, preferredCourseId?: string) => {
    ...
    setSectionForm(section && course ? {
      ...
      coTeacherIds: section.coTeacherIds || [], groupIds: section.groupIds || [], status: section.status || 'active',
  ...
  const selectedGroupStudentCount = courseStudentCount(students, sectionForm.groupIds);
  ...
  return <div className="min-w-0 space-y-5 text-left">
    <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
      <div><div className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-blue-600 text-white shadow-sm"><BookOpen className="h-4 w-4" /></span><h1 className="text-xl font-bold text-slate-950">จัดการรายวิชาและตอนเรียน</h1></div><p className="mt-1 text-xs text-slate-500">เพิ่มและจัดการข้อมูลรายวิชา ตอนเรียน ภาคการศึกษา ปีการศึกษา กลุ่มเรียน และอาจารย์ผู้สอน</p></div>
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-xs"><thead className="bg-slate-50 text-slate-600">{tab === 'courses' ? <tr><th className="px-4 py-3"><SortHeader value="code" label="รหัสวิชา" /></th><th className="px-4 py-3"><SortHeader value="name" label="ชื่อวิชา" /></th><th className="px-4 py-3">คณะที่รับผิดชอบ</th><th className="px-4 py-3">ภาควิชาที่รับผิดชอบ</th><th className="px-4 py-3 text-center">ตอนเรียน</th><th className="px-4 py-3 text-center">อาจารย์</th><th className="px-4 py-3 text-center">สถานะ</th><th className="px-4 py-3 text-right">การจัดการ</th></tr> : <tr><th className="px-4 py-3"><SortHeader value="code" label="รหัสวิชา" /></th><th className="px-4 py-3">ชื่อวิชา</th><th className="px-4 py-3"><SortHeader value="section" label="ตอนเรียน" /></th><th className="px-4 py-3"><SortHeader value="year" label="ภาค/ปี" /></th><th className="px-4 py-3">อาจารย์ผู้สอน</th><th className="px-4 py-3">กลุ่มเรียน</th><th className="px-4 py-3 text-center">นักศึกษา</th><th className="px-4 py-3 text-center">สถานะ</th><th className="px-4 py-3 text-right">การจัดการ</th></tr>}</thead>
        <tbody className="divide-y divide-slate-100">{tab === 'courses' ? (visible as Course[]).map((course) => {
          ...
          return <tr key={id} className="hover:bg-slate-50"><td className="px-4 py-4 font-mono font-bold text-blue-700">{course.courseCode}</td><td className="px-4 py-4">{course.courseName}</td><td className="px-4 py-4 font-semibold">ตอนเรียนที่ {section.sectionNo}</td><td className="px-4 py-4">{semesterLabel(section.semester)} / {section.academicYear}</td><td className="px-4 py-4"><strong>{primary?.fullName || 'ยังไม่มีอาจารย์'}</strong>{co.length > 0 && <div className="text-[10px] text-slate-500">ร่วม: {co.join(', ')}</div>}</td><td className="px-4 py-4"><div className="flex max-w-56 flex-wrap gap-1">{(section.groupIds || []).map((groupId) => <span key={groupId} className="rounded-md bg-blue-50 px-2 py-1 text-[10px] text-blue-700">{academicState.classGroups.find((group) => group.id === groupId)?.code || '—'}</span>)}</div></td><td className="px-4 py-4 text-center font-semibold">{courseStudentCount(students, section.groupIds || [])}</td><td className="px-4 py-4 text-center"><button type="button" aria-label={`${section.status === 'inactive' ? 'เปิด' : 'ปิด'}ใช้งานตอนเรียน ${section.sectionNo}`} title="คลิกเพื่อเปลี่ยนสถานะ" onClick={() => setConfirm({ type: 'sections', action: 'status', course, section })} className="rounded-full focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">{statusBadge(section.status)}</button></td><td className="px-3 py-3"><div className="flex justify-end">{actionIcon('ดูรายละเอียด', Eye, () => setDetail({ type: 'sections', course, section }))}{actionIcon('แก้ไข', Pencil, () => openSection({ course, section }))}{actionIcon('จัดการอาจารย์', GraduationCap, () => openSection({ course, section }))}{actionIcon('จัดการกลุ่มเรียน', Users, () => openSection({ course, section }))}{actionIcon('ลบ', Trash2, () => setConfirm({ type: 'sections', action: 'delete', course, section }))}</div></td></tr>;
    ...
    <Modal isOpen={Boolean(sectionEditor)} onClose={() => setSectionEditor(null)} title={sectionEditor === 'new' ? 'เปิดตอนเรียนใหม่ (New Section)' : 'แก้ไขตอนเรียน'} maxWidth="wide"><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); const result = saveSectionRecord(sectionForm, editorSectionId); if (result.success) setSectionEditor(null); else setError(result.error || ''); }}>
      ...
      <fieldset><div className="mb-1 flex items-center justify-between gap-3"><legend className="text-xs font-semibold text-slate-700">กลุ่มเรียนที่เข้าเรียน (Class Groups / Cohorts) <span className="text-red-500">*</span></legend><span className="text-[10px] font-semibold text-blue-600">รวมนักศึกษาประมาณ {selectedGroupStudentCount} คน ({sectionForm.groupIds.length} กลุ่ม)</span></div><div className="max-h-40 overflow-y-auto rounded-xl border border-slate-200 bg-slate-50/50">{academicState.classGroups.filter((group) => { const path = academicPath(academicState, 'classGroups', group.id); return isAcademicPathActive(academicState, 'classGroups', group.id) && path.department?.id === sectionDepartmentId; }).map((group) => { const path = academicPath(academicState, 'classGroups', group.id); const count = students.filter((student) => student.accountStatus === 'active' && student.classGroupId === group.id).length; return <label key={group.id} className="flex cursor-pointer items-center gap-3 border-b border-slate-100 px-5 py-3 text-xs last:border-b-0 hover:bg-white"><input type="checkbox" checked={sectionForm.groupIds.includes(group.id)} onChange={(event) => setSectionForm({ ...sectionForm, groupIds: event.target.checked ? [...sectionForm.groupIds, group.id] : sectionForm.groupIds.filter((id) => id !== group.id) })} /><strong>{group.code}</strong><span className="text-slate-500">ปีเข้า {group.admissionYear} • {path.year?.name || '—'}</span><span className="ml-auto rounded-lg border border-slate-200 bg-white px-2 py-1 text-[10px] text-slate-500">{count} คน</span></label>; })}</div><p className="mt-1 text-[10px] text-slate-400">ระบบมีระบบป้องกันข้อขัดแย้ง: กลุ่มเรียนไม่สามารถลงทะเบียนซ้ำในรายวิชาเดียวกันในภาคการศึกษาเดียวกันได้</p></fieldset>
...
const SectionSummary: React.FC<{ course: Course; section: Section; teachers: ReturnType<typeof useApp>['teachers']; students: ReturnType<typeof useApp>['students']; academicState: ReturnType<typeof useApp>['academicState'] }> = ({ course, section, teachers, students, academicState }) => <dl className="grid gap-3 sm:grid-cols-2">{[
  ...
  ['กลุ่มเรียน', (section.groupIds || []).map((id) => academicState.classGroups.find((group) => group.id === id)?.code).filter(Boolean).join(', ') || '—'],
  ['จำนวนนักศึกษา', `${courseStudentCount(students, section.groupIds || [])} คน`],
